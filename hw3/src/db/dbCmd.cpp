...
DBAppendCmd::exec(const string& option)
{
   ...
   if(!CmdExec::lexOptions(option, tokens, 2))  return CMD_EXEC_ERROR;
   else{
      if(!isValidVarName(tokens[0])){
         CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[0]);
         return CMD_EXEC_ERROR;
      }
      else if (!myStr2Int(tokens[1], value)){
         CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[1]);
         return CMD_EXEC_ERROR;
      }
      else{
         for(size_t i = 0; i < dbjson.size(); i++){//check for same key
            if(dbjson[i].key() == tokens[0]){
               cerr << "Error: Element with key \"" << tokens[0] << "\" already exists!!" << endl;
               return CMD_EXEC_ERROR;
            }
         }
         DBJsonElem temp(tokens[0], value);
         dbjson.add(temp);
      }
   }
...
{  
   ...
   if(CmdExec::lexSingleOption(option, tok, true)){
      if(tok.size() == 0){
         cout << dbjson;
         cout << endl << "Total JSON elements: " << dbjson.size() << endl;
...
DBReadCmd::exec(const string& option)
{
   ...
   //if (!(ifs >> dbjson)) return CMD_EXEC_ERROR;
   ifs >> dbjson;
